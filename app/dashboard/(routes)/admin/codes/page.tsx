...
import { downloadCodesXlsx, mapAdminCodesToRows } from "@/lib/export/codes-to-xlsx";
...
const AdminCodesPage = () => {
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const data = await response.json();
      const rows = mapAdminCodesToRows(Array.isArray(data) ? data : []);
      const date = format(new Date(), "yyyy-MM-dd", { locale: ar });
      await downloadCodesXlsx({
        filename: `admin-codes-${date}.xlsx`,
        sheetName: "Codes",
        rows,
      });
